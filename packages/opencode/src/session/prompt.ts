  ...
  Effect.gen(function* () {
    ...
    const runLoop: (input: LoopInput) => Effect.Effect<SessionV1.WithParts> = Effect.fn("SessionPrompt.run")(
      function* (input: LoopInput) {
        const { sessionID, personality } = input
        const ctx = yield* InstanceState.context
        ...
        while (true) {
          ...
          const outcome: "break" | "continue" = yield* Effect.gen(function* () {
            ...
            const lastUserMessage = msgs.findLast((message) => message.info.id === lastUser.id)
            const memoryQuery =
            ...
            if (result === "stop" && handle.message.error && isModelLimitError(handle.message.error)) {
              ...
              let fallback: Provider.Model | undefined
              for (const candidate of candidates) {
                const key = `${candidate.providerID}/${candidate.modelID}`
                if (attemptedFallbacks.has(key)) continue
                const resolved = yield* provider
                  .getModel(ProviderV2.ID.make(candidate.providerID), ModelV2.ID.make(candidate.modelID))
                  .pipe(Effect.option)
                if (Option.isSome(resolved) && !sameModel(model, resolved.value)) {
                  fallback = resolved.value
                  break
                }
              }
              if (fallback) {
                const fallbackRef = { providerID: fallback.providerID, modelID: fallback.id }
                attemptedFallbacks.add(`${fallback.providerID}/${fallback.id}`)
                yield* sessions.updateMessage({ ...lastUser, model: fallbackRef })
                yield* sessions.setAgentModel({
                  sessionID,
                  agent: lastUser.agent,
                  model: { id: fallback.id, providerID: fallback.providerID, variant: "default" },
                  time: Date.now(),
                })
                yield* Effect.promise(() =>
                  recordLuisMemory({
                ...
                yield* Effect.logWarning("model fallback", {
                  ...
                  to: `${fallback.providerID}/${fallback.id}`,
                })
