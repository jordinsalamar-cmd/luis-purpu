...
function pushBrand(
  ...
  chars.forEach((char, index) => {
    ...
    push(lines, left + index, top, char, RGBA.fromIndex(color), undefined, attrs)
  })
}
...
function build(input: SplashWriterInput, kind: "entry" | "exit", ctx: ScrollbackRenderContext): ScrollbackSnapshot {
  ...
  if (kind === "entry") {
    ...
    pushBrand(lines, body_left, top, APP_DISPLAY_NAME, TextAttributes.BOLD)
    push(lines, body_left, top + 1, `Creador: ${CREATOR_NAME}`, left, undefined, TextAttributes.DIM)
    if (input.detail) {
      push(
        lines,
        body_left,
  ...
  if (kind === "exit") {
    ...
    push(
      ...
      top + 1,
      `${CLI_COMMAND} --mini -s ${meta.session_id}`,
      right,
      undefined,
      TextAttributes.BOLD,
    )
