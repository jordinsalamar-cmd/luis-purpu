...
import { takeLuisInput } from "./luis-bridge"
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  createEffect(() => {
    ...
    if (route.data.type === "home") {
      renderer.setTerminalTitle(APP_DISPLAY_NAME)
      return
    }
    ...
    if (route.data.type === "session") {
      ...
      renderer.setTerminalTitle(`REM | ${title}`)
      return
    }

    if (route.data.type === "plugin") {
      renderer.setTerminalTitle(`REM | ${route.data.id}`)
    }
  })
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to ${APP_DISPLAY_NAME} v${result.data.version}. Please restart the application.`,
    )
