...
Agente de escritorio para Windows con terminal interactiva, memoria persistente, grafo del proyecto, voz femenina online y local, escucha por micrófono, companion VRM y automatización del equipo.
...
```powershell
git clone https://github.com/jordinsalamar-cmd/luis-purpu.git rem
cd rem
...
```powershell
$z="$env:TEMP\rem.zip"; irm https://github.com/jordinsalamar-cmd/luis-purpu/archive/refs/heads/master.zip -OutFile $z; Expand-Archive $z -DestinationPath "$env:USERPROFILE\rem" -Force; cd "$env:USERPROFILE\rem\luis-purpu-master"; powershell -ExecutionPolicy Bypass -File .\install.ps1
```
...
- `packages/opencode/resources/luis-companion/assets/rem.vrm`: cuerpo portable de Rem.
...
- `graphify-out/rem-memory.json`: memoria persistente entre sesiones.
...
El reconocimiento del micrófono usa el modelo Vosk incluido y puede funcionar sin internet. Con conexión, la voz usa `edge-tts` con `es-MX-DaliaNeural` (femenina, cálida); sin conexión, usa Piper local y Windows como respaldo femenino.
...
La interfaz, el comando público y la identidad visible son Rem. El repositorio conserva su dirección histórica para que las instalaciones existentes puedan actualizarse.
